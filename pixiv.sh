function pixiv { # $1 = id
    ...
    do
        ...
        then
            ...
            do
                [ "$1" = "verbose" ] && echo -e "    \e[32m$zip\e[36m added into aria2 list\e[0m"
                echo "$zip" >> list
            done
        else
            [ "$1" = "verbose" ] && echo -e "    \e[32m$links\e[36m added into aria2 list\e[0m"
            echo "$links" >> list
        fi
    done
}
...
function pixiv_by_artist { # $1 = artist id
    total=`eval "curl 'https://www.pixiv.net/ajax/user/$1/profile/all?lang=en' $parameters" | grep -Po '"illusts":{.*?}' | sed 's/"/\n/g' | grep -Ec "[0-9]+"`
...
function pixiv_by_artist_and_tags { # $1 = artist id, $2 = tags
    tags="${2// /%20}"
    total=`eval "curl 'https://www.pixiv.net/ajax/user/$1/illusts/tag?tag=$tags&offset=0&limit=48&lang=en' $parameters" | grep -Eo '"total":[0-9]+' | grep -Eo "[0-9]+"`
    echo -e "\e[36mFound \e[32m$total\e[36m illustrations\e[0m"
    current=0
